...
class RSSMDynamicsBase(nn.Module):
    ...
    # TODO
    def imagine(self, prev_state: dict, x_action_batch: torch.Tensor, history_horizon: int = -1):
        # History warmup: process the full context window with observe().
        # is_first is 1 for the very first timestep, 0 for the rest.
        pred = self.rssm.imagine_with_action(x_action_batch[:, history_horizon:], prev_state)
        
        return self.rssm.get_feat(pred)
    ...
    def forward(self, x_state_batch: torch.Tensor, x_action_batch: torch.Tensor) -> torch.Tensor:
        ...
        """
        if x_state_batch.dim() == 2:
            x_state_batch = x_state_batch.unsqueeze(1)
            x_action_batch = x_action_batch.unsqueeze(1)
        # History warmup: process the full context window with observe().
        # is_first is 1 for the very first timestep, 0 for the rest.
        batch_size = x_state_batch.shape[0]
        is_first = torch.zeros(batch_size, x_state_batch.shape[1], device=self.device)
        is_first[:, 0] = 1.0
        ...
        if x_state_batch.shape[1] != 1:
            self.last_post = {k: v[:, -1] for k, v in post.items()}
            return self.rssm.get_feat(prior)[:, -1]
        else:
            assert self.last_post is not None
            return self.rssm.get_feat(self.rssm.img_step(self.last_post, x_action_batch[:, -1].squeeze(1)))
