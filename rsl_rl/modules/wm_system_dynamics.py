...
from rsl_rl.modules.architectures import MLP, MLPBase, RNNBase, MLPStateHead, MLPAuxiliaryHead, RSSMBase, MultiEncoder, MultiDecoder

class RWMPSystemDynamicsEnsemble(nn.Module):
    def __init__(
        self,
        state_dim: int,
        action_dim: int,
        ...
        self.decoder_config = head_config["decoder"]
        self.reward_config = head_config["reward"]
    ...
    def _init_networks(self):
        ...
        self.dynamic = RSSMBase(
            input_dim=self.encoder.outdim,
            device=self.device,
            architecture_config=self.dynamics_config
        )
        self.reward_config["device"] = self.device
    ...
    def _create_base(self):
        if self.forecast_config["type"] == "mlp":
            input_dim = self.history_horizon * (self.state_dim + self.action_dim)
            self.base_output_dim = self.forecast_config["base_shape"][-1]
            self.prediction_type = "single"
            return MLPBase(
                input_dim=input_dim,
                device=self.device,
                architecture_config=self.forecast_config,
            )
        elif self.forecast_config["type"] == "rnn":
            input_dim = self.state_dim + self.action_dim
            self.base_output_dim = self.forecast_config["rnn_hidden_size"]
            self.prediction_type = "single"
            return RNNBase(
                input_dim=input_dim,
                device=self.device,
        ...
        elif self.forecast_config["type"] == "rssm":
            input_dim = self.encoder.outdim
            self.base_output_dim = self.forecast_config["hidden"]
            self.prediction_type = "single"
            return RSSMBase(
                input_dim=input_dim,
                device=self.device,
    ...
    def compute_state_loss(self, head, state_batch, action_batch):
        ...
        for i in range(forecast_horizon):
            ...
            if self.forecast_config["type"] in ["rnn", "rssm"] and i > 0:
            ...
            kl_loss = self.state_base.kl_loss if self.forecast_config["type"] == "rssm" else torch.tensor(0.0, device=self.device)
            ...
            if self.forecast_config["type"] in ["rnn", "rssm"]:
