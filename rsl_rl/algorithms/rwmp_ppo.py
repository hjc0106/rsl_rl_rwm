...
from rsl_rl.modules import RWMPSystemDynamicsEnsemble
from rsl_rl.modules.rnd import RandomNetworkDistillation
from rsl_rl.storage import RolloutStorage, RWMPRolloutStorage
...
class RWMPPPO(PPO):
    ...
    def __init__(
        ...
        # World model (RSSM) optimizer parameters; default to system_dynamics values if not provided
        ...
        # override the storage and transition
        self.vel_predict_coef = vel_predict_coef
        self.storage: RWMPRolloutStorage = None  # type: ignore
        self.transition = RWMPRolloutStorage.Transition()

        self.imagination_storage = None
        ...
        # RSSM world model components: encoder + RSSM dynamics + decoder/reward heads
        self._wm_params = (
            list(self.system_dynamics.encoder.parameters())
            + list(self.system_dynamics.dynamic.parameters())
            + list(self.system_dynamics.heads.parameters())
        )
        # MLP/RNN state-prediction components (filtered by requires_grad to respect freeze_auxiliary)
        self._sd_params = [
            p for p in (
                list(self.system_dynamics.state_base.parameters())
                + list(self.system_dynamics.state_heads.parameters())
                + list(self.system_dynamics.auxiliary_base.parameters())
                + list(self.system_dynamics.auxiliary_heads.parameters())
            )
            if p.requires_grad
        ]
        _wm_lr = wm_learning_rate if wm_learning_rate is not None else system_dynamics_learning_rate
        _wm_wd = wm_weight_decay if wm_weight_decay is not None else system_dynamics_weight_decay
        self.wm_learning_rate = _wm_lr
        self.wm_use_amp = wm_use_amp
        # Optimizer for MLP-based state/auxiliary prediction (updated at policy frequency)
        self.system_dynamics_optimizer = optim.Adam(
            self._sd_params, lr=system_dynamics_learning_rate, weight_decay=system_dynamics_weight_decay
        )
        # Optimizer for RSSM world model (updated at lower frequency via update_world_model)
        self.wm_optimizer = tools.Optimizer(
            name="world_model",
            parameters=self._wm_params,
            lr=_wm_lr,
            eps=wm_opt_eps,
            clip=wm_max_grad_norm,
            wd=_wm_wd,
            opt=wm_opt_type,
            use_amp=wm_use_amp,
        )
    ...
    def init_storage(self, training_type, num_envs, num_transitions_per_env, obs, actions_shape, imagination=False):
        if imagination:
            ...
            self.imagination_storage = RWMPRolloutStorage(
                training_type,
                num_envs,
                num_transitions_per_env,
                imagination_obs,
                actions_shape,
                self.policy.history_dim,
                self.policy.wm_feature_dim,
                self.device,
            )
        else:
            self.storage = RWMPRolloutStorage(
                training_type,
                num_envs,
                num_transitions_per_env,
                obs,
    ...
    def get_wm_feature(self, wm_obs, wm_action, wm_latent):
        """ 
        Get the world model feature from the observations.
        Args:   
            wm_obs: The world model observations.
            wm_action: The world model action.
            wm_latent: The world model latent.
        Returns:
            wm_feature: The world model feature.
        """
        wm_embed = self.system_dynamics.encoder(wm_obs)
        wm_latent, _ = self.system_dynamics.dynamic.obs_step(wm_latent, wm_action, wm_embed, wm_obs["is_first"])
        wm_feature = self.system_dynamics.dynamic.get_deter_feat(wm_latent)
        return wm_latent, wm_feature

    # override the act method
    def act(self, obs, history, wm_feature):
        if self.policy.is_recurrent:
            self.transition.hidden_states = self.policy.get_hidden_states().detach()
        # Compute the actions and values
        self.transition.history = history.detach()
        self.transition.wm_feature = wm_feature.detach()
        self.transition.actions = self.policy.act(obs, history, wm_feature).detach()
        # self.policy.eval()
        self.transition.values = self.policy.evaluate(obs, wm_feature).detach()
        # self.policy.train()
        ...
        self.transition.critic_observations = self.policy.get_critic_obs(obs).detach()
    ...
    def compute_returns(self, obs, wm_feature, imagination=False):
        if imagination:
            # compute value for the last step
            last_values = self.policy.evaluate(obs, wm_feature).detach()
    ...
    def update_world_model(self, wm_data):
        # WMP Loss
        data = self.preprocess(wm_data)

        with torch.cuda.amp.autocast(enabled=self.wm_use_amp):
            embed = self.system_dynamics.encoder(data)
            post, prior = self.system_dynamics.dynamic.observe(
                embed, data["action"], data["is_first"]
            )
            kl_free = self.system_dynamics.dynamics_config["kl_free"]
            dyn_scale = self.system_dynamics.dynamics_config["dyn_scale"]
            rep_scale = self.system_dynamics.dynamics_config["rep_scale"]
            kl_loss, kl_value, dyn_loss, rep_loss = self.system_dynamics.dynamic.kl_loss(
                post, prior, kl_free, dyn_scale, rep_scale
            )
            assert kl_loss.shape == embed.shape[:2], kl_loss.shape
            preds = {}
            for name, head in self.system_dynamics.heads.items():
                grad_head = name in self.system_dynamics.head_config["grad_heads"]
                feat = self.system_dynamics.dynamic.get_feat(post)
            ...
            for name, pred in preds.items():
                ...
                losses[name] = loss
            scaled = {
                key: value * self.system_dynamics.head_config["loss_scale"].get(key, 1.0)
                for key, value in losses.items()
            }
            model_loss = sum(scaled.values()) + kl_loss
            mean_model_loss = torch.mean(model_loss)
        metrics = self.wm_optimizer(mean_model_loss, self._wm_params)
        ...
        with torch.cuda.amp.autocast(self.wm_use_amp):
            metrics["prior_ent"] = to_np(
                torch.mean(self.system_dynamics.dynamic.get_dist(prior).entropy())
            )
            metrics["post_ent"] = to_np(
                torch.mean(self.system_dynamics.dynamic.get_dist(post).entropy())
            )
            context = dict(
                embed=embed,
                feat=self.system_dynamics.dynamic.get_feat(post),
                kl=kl_value,
                postent=self.system_dynamics.dynamic.get_dist(post).entropy(),
            )
    ...
    def update_system_dynamics(self):
        ...
        for system_state_batch, system_action_batch, system_extension_batch, system_contact_batch, system_termination_batch in system_generator:
            ...
            loss.backward()
            nn.utils.clip_grad_norm_(self._sd_params, self.max_grad_norm)
    ...
    def system_dynamics_autoregressive_prediction(self, state_traj, action_traj, extension_traj, contact_traj, termination_traj):
        ...
        with torch.inference_mode():
            for i in range(self.system_dynamics.history_horizon, self.system_dynamics_len_eval_trajectory):
                if self.system_dynamics.forecast_config["type"] in ["rnn", "rssm"] and i > self.system_dynamics.history_horizon:
    ...
    def update(self, imagination=False):  # noqa: C901
        ...
        for (
            ...
            # -- actor
            self.policy.act(obs_batch, history_batch, wm_feature_batch, masks=masks_batch, hidden_states=hid_states_batch[0])
            actions_log_prob_batch = self.policy.get_actions_log_prob(actions_batch)
            # -- critic
            value_batch = self.policy.evaluate(obs_batch, wm_feature_batch, masks=masks_batch, hidden_states=hid_states_batch[1])
            # -- entropy
            ...
            predicted_linear_vel = self.policy.get_linear_vel(obs_batch, history_batch)
            target_linear_vel = self.policy.get_critic_obs(obs_batch)[:, self.policy.privileged_dim - 3: self.policy.privileged_dim]
        ...
        loss_dict = {
            ...
            "vel_predict": mean_vel_predict_loss,
        }
        if self.rnd:
